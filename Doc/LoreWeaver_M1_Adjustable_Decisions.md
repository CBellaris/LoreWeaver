...
- embedding input 当前由 `micro_topic + micro_summary + entities + topics` 组成；
...
- BM25 文档由 `micro_topic + micro_summary + entities + topics + key_quote` 拼接；
...
- BM25 字段权重：entities > micro_topic > topics > summary > key_quote；
- BM25 query 侧实体/别名扩展；

...
- `micro_summary`：1-2 句，只概括当前小 Span，不写窗口或整章总结；
...
2. 实现 Span 写入与查询。
3. 选择 embedding 输入：
   - M1 默认使用 `micro_topic + micro_summary + entities + topics`；
   - 可选加入 `key_quote`。
4. 批量生成 embedding。
5. 写入 Qdrant：
   - vector；
...
- embedding 输入太短：拼接 micro_topic、micro_summary、entities、topics；
...
- 实现 embedding 输入构建与缓存键逻辑，M1 默认输入为 `micro_topic + micro_summary + entities + topics`，可通过配置控制是否加入 `key_quote` 或 `located_text`；
...
- Reranker 吃不下长文本：使用 micro_topic + micro_summary + key_quote + entities 作为精排文本。

...
def _sample_spans(document_id: str) -> list[Span]:
    ...
    spans: list[Span] = []
    rows = [
        ("power_rule", "精神视角规则", "高文以漂浮视角观察世界，精神状态遵循异常规则。", ["高文"], ["精神规则", "观察限制"]),
        ("power_rule", "观察距离限制", "高文发现视角固定，无法随意移动。", ["高文"], ["精神规则", "观察限制"]),
        ("power_rule", "意识维持方式", "高文依靠持续思考维持自我。", ["高文"], ["精神规则", "生存危机"]),
        ("power_rule", "异常能力边界", "异常能力并非万能，只能记录和观察。", ["高文"], ["精神规则", "能力边界"]),
        ("power_rule", "精神规则转折", "精神状态濒临消散时出现新的转折。", ["高文"], ["精神规则", "转折点"]),
        ("mystery_clue", "火种时间异常", "火种诞生后，高文的时间感知发生异常。", ["高文", "火种"], ["时间异常", "伏笔"]),
        ("mystery_clue", "逃逸程序提示", "神秘声音提示逃逸程序启动。", ["高文"], ["逃逸程序", "伏笔"]),
        ("mystery_clue", "黑钢棺材怪响", "黑钢棺材内传来怪响，众人感到不安。", ["黑钢棺材", "瑞贝卡"], ["异常现象", "伏笔"]),
